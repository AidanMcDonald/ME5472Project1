...
% Feb/2021 - added line 425 since otherwise a boundary line gets dropped!
...
BS=findboundary(M,Xp,Yp); %%%% add Xp,Yp
iBS=find(isnan(BS));
% Third step:
...
for l=1:length(nv)   % For each level
  ...
  if any(iCSlvl)
    ...
    %%     line(xseg,yseg,'color','m','linewi',2);pause;
...
varargout={fCS,H};
%-----------------------------------------------------------------
function BS=findboundary(M,Xp,Yp)
...
% sine neighbouring traingles will traverse the same
% segment in opposite directions, first
...
BS(iBS)=NaN;  % Added Feb/2021 to handle holes and things!
BS(iBS+1:end)=[];
